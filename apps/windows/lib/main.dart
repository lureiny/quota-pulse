...
class QuotaPulseApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(useMaterial3: true, colorSchemeSeed: const Color(0xFF3478F6)),
...
class _ShellState extends State<Shell> with TrayListener, WindowListener {
  ...
  void _onPulse() {
    ...
    // Windows 托盘没有标题文字,峰值放进 tooltip
    trayManager.setToolTip(
      peak == null ? 'quota-pulse 用量' : 'quota-pulse · 峰值 ${(peak * 100).round()}%',
    );
    if (mounted) setState(() {});
  }
