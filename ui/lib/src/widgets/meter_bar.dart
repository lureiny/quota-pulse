...
/// 「重置时间」段:一段与副标同色的裸文字。
///   悬停 → [Tooltip] 浮窗显示 [other](另一种时间格式)预览;
///   单击 → [onToggle](翻转全局 resetMode)。
/// [other] 为空(如只有相对无绝对时刻)则退化为纯文本,不预览也不可切换。
class _ResetChunk extends StatelessWidget {
  const _ResetChunk({
    required this.reset,
    required this.other,
    required this.style,
  ...
  final String other; // 另一种格式(悬停浮窗预览);为空则无预览
  final TextStyle style; // 与副标同款(_subStyle)
  final VoidCallback? onToggle;

  @override
  Widget build(BuildContext context) {
    final text = Text(reset, style: style);
    ...
    if (onToggle != null) {
      w = MouseRegion(
        ...
        child: GestureDetector(onTap: onToggle, child: w),
      );
    }
    return Tooltip(
      message: other,
