...
- **菜单栏 / 托盘**:macOS 自绘 `NSStatusItem`(固定图标 + 文字溢出时像素级滚动,宽度/速度可配);Windows 每账户两行 tooltip;无论模式如何**始终展示 5h 窗口**;可选显示用量或剩余量;范围支持全部账户(默认)或多选钉住子集。
- **用量提醒**:两类可独立配置的通知,每类各自可选监听 **5h / 7d** 窗口——**超阈值**(默认开,5h+7d;🚨 告警 → 🛑 用量已满)与**额度恢复**(默认关,✅ 回落到阈值以下/窗口重置时);同类通知一个重置周期内只弹一次;首次快照静默;带系统提示音;设置页「发送测试通知」一键依次弹出三种样例(🚨/🛑/✅)核验送达。
...
- **Go 核心**:被动/主动自适应轮询(60s/10m)、ETag/304 条件请求、空响应防回退缓存、限流并发(≤6)、Provider 插件化、`dart:ffi` + gomobile 桥、`qpctl` 调试 CLI。
